...
public class ShopsService {
    ...
    public Person readManager(String id){
    	return people.readPerson(id);
    }
    ...
    public ResponseData remove(final String shopName) throws ShopsException {
        if (StringUtils.isEmpty(shopName)) {
        	log.error("Invalid name, which is null or empty");
    ...
    public ResponseData save(final Shop shop) throws ShopsException {
        try {
            if (validateShop(shop)) {
                final ResponseData response = repository.save(shop);
                geocodingService.obtainLatittudeLongitude(shop).thenAccept(this::includeCoordinates);
                return response;
            } else {
            	log.error("Invalid Shop= %s", shop.toString());
    ...
    public Shop findNearestShops(final LatLng latLng) {
        ...
        log.info("The nearest shop is %s", nearestShop.toString());
        return nearestShop;
    }
}
